...
elseif iscell(func_name)
    for i = 1:numel(func_name)
        checkFunctionName(func_name{i});
    end
else
...
if A == 2 || A == 5
    url = ['https://uk.mathworks.com/help/matlab/ref/' fname '.html'];
    [str,status] = urlread(url);
    if isempty(str)
        url = ['https://uk.mathworks.com/help/simulink/slref/' fname '.html'];
        [str,status] = urlread(url);
    end
    if status == 0
